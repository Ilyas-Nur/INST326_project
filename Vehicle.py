...
class Vehicle:
    """
        This class enables us to create vehicle objects
         ...
         Attributes:
            ...
            - type (str): Type of vehicle (sedan, hatchback, truck, SUV, van, 
                coupe)
    ...
    def __init__(self, vehicle_info):
        """
            Takes a single line of text as an argument when instantiating the 
            class and sets the attributes.
            ...
            Raises:
                ValueError: Indicates when the vehicle_info string could not be 
                parsed
        """
        ...
        if re.search(regex, vehicle_info):
            ...
            self.car_name = match.group("Car_Name") # Captures Year, Make, Model
            self.year = int(match.group("Year"))
            self.make = match.group("Make")
            self.model = match.group("Model")
            self.type = match.group("Car_Type")
            self.fuel_type = match.group("Fuel_Type")
            self.msrp = int(match.group("MSRP")[1:])
